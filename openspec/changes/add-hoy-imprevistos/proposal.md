...
- **Borra**: la subtask al BORRAR un imprevisto en Hermes (semántica: "esto no
  debió cargarse" — tampoco debe quedar basura en Mesa Técnica), y en el
  fallback de promoción, si el MCP no soporta quitar parent — se borra la
  subtask y se crea la tarea top-level en su lugar (mismo título, nueva
  tarea). Nada más se borra jamás.
