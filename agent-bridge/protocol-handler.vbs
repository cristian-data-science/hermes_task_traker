...
If Len(path) > 4 And (Mid(path, 2, 2) = ":\" Or Left(path, 2) = "\\") Then
  ...
  ElseIf mode = "md" Then
    ...
    Else
      ...
      MsgBox "No hay ningún reporte .md generado desde el inicio de la corrida." & vbCrLf & vbCrLf & _
             "Carpeta revisada:" & vbCrLf & path & vbCrLf & vbCrLf & _
             "La corrida probablemente no creó un reporte. Revisa el resultado en la app.", _
